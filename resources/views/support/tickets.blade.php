                ...
                <form method="POST" action="{{ route('support.tickets.store') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="support-name" name="name" type="text" autocomplete="name"
                            class="mt-2 block w-full rounded-lg border-gray-300 shadow-xs focus:border-green-600 focus:ring-green-600"
                            value="{{ $name }}">
                        @error('name')
                    ...
                    <div>
                        ...
                        <textarea id="support-message" name="message" rows="6"
                            class="mt-2 block w-full rounded-lg border-gray-300 shadow-xs focus:border-green-600 focus:ring-green-600">{{ $supportMessage }}</textarea>
                        @error('message')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="flex items-center justify-end">
                        <button type="submit"
                            class="inline-flex items-center justify-center rounded-lg bg-green-700 px-5 py-2.5 text-sm font-semibold text-white shadow-xs transition hover:bg-green-600 focus:outline-hidden focus:ring-2 focus:ring-green-600 focus:ring-offset-2">
