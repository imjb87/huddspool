            ...
            <div class="border-b border-gray-200 bg-linear-to-b from-gray-50 to-gray-100 dark:border-zinc-800/80 dark:from-zinc-900 dark:via-zinc-900 dark:to-zinc-800/80">
                <div class="mx-auto flex w-full max-w-4xl" data-section-table-band>
                    ...
                    <div class="flex w-[56%] pr-4 sm:w-1/2 sm:pr-6">
                        <div scope="col" class="w-1/5 py-2 text-right text-sm font-semibold text-gray-900 dark:text-gray-100">Pl</div>
                        <div scope="col" class="w-1/5 py-2 text-right text-sm font-semibold text-gray-900 dark:text-gray-100">W</div>
                        <div scope="col" class="w-1/5 py-2 text-right text-sm font-semibold text-gray-900 dark:text-gray-100">D</div>
                        <div scope="col" class="w-1/5 py-2 text-right text-sm font-semibold text-gray-900 dark:text-gray-100">L</div>
                        <div scope="col" class="w-1/5 py-2 text-right text-sm font-semibold text-gray-900 dark:text-gray-100">Pts</div>
                    </div>
                </div>
            </div>
            <div class="bg-white dark:bg-zinc-900/75">
                ...
                @else
                    @foreach ($standings as $index => $team)
                        ...
                        @endphp
                        @if ($canLinkTeam)
                            <a class="block w-full border-t border-gray-300 hover:cursor-pointer hover:bg-gray-50 dark:border-zinc-800/80 dark:hover:bg-zinc-800/70 {{ $withdrawn ? 'line-through' : '' }}"
                                wire:key="section-standing-{{ $section->id }}-{{ $team->id }}"
                                data-section-table-row-type="link"
                                href="{{ route('team.show', $team->id) }}">
                        @else
                            <div class="block w-full border-t border-gray-300 dark:border-zinc-800/80 {{ $withdrawn ? 'line-through' : '' }}"
                                wire:key="section-standing-{{ $section->id }}-{{ $team->id }}"
                        ...
                        @endif
                            <div class="mx-auto flex w-full max-w-4xl" data-section-table-band>
                                ...
                                <div class="flex w-[56%] items-center pr-4 sm:w-1/2 sm:pr-6">
                                    ...
                                    <div class="w-1/5 py-2 text-right text-sm font-semibold {{ $pointsClass }}">
                                        {{ $team->points }}
                                    </div>
                                </div>
                            </div>
