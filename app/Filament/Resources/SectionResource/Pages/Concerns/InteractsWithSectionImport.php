...
namespace App\Filament\Resources\SectionResource\Pages\Concerns;
...
trait InteractsWithSectionImport
{
    ...
    protected function getSectionImportFormSchema(): array
    {
        return [
            Section::make('Source data')
                ...
                ->schema([
                    ...
                    Forms\Components\Select::make('ruleset_id')
                        ->label('Ruleset')
                        ->options(fn (): array => Ruleset::query()->orderBy('name')->pluck('name', 'id')->toArray())
                        ->searchable()
                        ->preload()
                        ->required(),
            ...
            Section::make('Teams')
                ...
                ->visible(fn (Get $get): bool => filled($get('teams')))
                ->schema([
                    Forms\Components\Repeater::make('teams')
                        ...
                        ->schema([
                            ...
                            Forms\Components\Select::make('team_id')
                                ->label('Existing team')
                                ->options(fn (): array => Team::query()->orderBy('name')->pluck('name', 'id')->toArray())
                                ->searchable()
                                ->preload()
                                ->native(false)
                            ...
                            Forms\Components\TextInput::make('new_team_name')
                                ...
                                ->columnSpanFull()
                                ->visible(fn (Get $get): bool => blank($get('team_id')))
                                ->required(fn (Get $get): bool => blank($get('team_id'))),
                            Forms\Components\Select::make('new_team_venue_id')
                                ->label('Venue')
                                ->placeholder('Select a venue for the new team')
                                ->options(fn (): array => Venue::query()->orderBy('name')->pluck('name', 'id')->toArray())
                                ->searchable()
                                ->preload()
                                ->columnSpanFull()
                                ->visible(fn (Get $get): bool => blank($get('team_id')))
    ...
    protected function hydrateTeamsFromCsv(Set $set, ?string $state): void
    {
        ...
        $set('teams', collect($parsed->teams)
            ->map(function (array $team, int $index): array {
    ...
    protected function handleImportSection(array $data): void
    {
        ...
        $season = $this->getContextSeason();
        $section = null;

        DB::transaction(function () use ($teams, $ruleset, $season, $sectionName, &$section): void {
    ...
    protected function getContextSeason(): Season
    {
        $season = $this->getParentRecord();
        ...
        throw ValidationException::withMessages([
            'section_name' => 'Select a season before importing sections.',
        ]);
    }
}
